...
def split_string_to_messages(string):
	...
	if('/' in string[:8]):
		...
		print('detected mm.dd.yy dateformat (using dateseprarator: ' + dateseprarator + ')')
	else:
...
def to_telegram_format(messages):
	...
	data['chats']['list'][0]['name'] = messages[0].split('- ')[1].split(':')[0] # name of the first message
	...
	for message in messages:
		...
		data['chats']['list'][0]['messages'][-1]['date'] = str(date_y).zfill(2)  + '-' + str(date_m).zfill(2)  + '-' + str(date_d).zfill(2)  + 'T' + str(time_h).zfill(2)  + ':' + str(time_m).zfill(2)  + ':00'  # 2017-06-12T21:30:09
		...
		data['chats']['list'][0]['messages'][-1]['photo'] = 'placeholder/path/to/photo.jpg'
		id += 1
	return data
